...
def _animated_to_spritesheet(
    ...
    if not backend:
        raise RuntimeError("Not detected ImageMagick/ffmpeg，Cannot auto-convert animation")
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if frames <= 0:
            ...
            if os.system(cmd1) != 0:
                raise RuntimeError("Animation frame extraction failed（Pillow/ffmpeg all failed）")
        ...
        if backend == "magick":
            ...
            rc = os.system(cmd)
            if rc != 0:
                raise RuntimeError("ImageMagick Tile assembly failed")
        ...
        if os.system(cmd2) != 0:
            raise RuntimeError("ffmpeg Tile assembly failed")
...
def _generate_rpg_background_to_webp(out_webp_path: str, width: int = 1280, height: int = 720, custom_prompt: str = "", speed_mode: str = "fast"):
    ...
    if not (os.path.exists(GEMINI_PYTHON) and os.path.exists(GEMINI_SCRIPT)):
        raise RuntimeError("Image gen script env missing：gemini-image-generate not installed")

    style_hint = (custom_prompt or "").strip()
    if not style_hint:
        style_hint = theme

    #  quality  fast  API unavailable
    ...
    #  400 
    #  nanobanana-2  aspect-rationanobanana-pro  1280x720
    ...
    for mname in model_candidates:
        ...
        # unavailable
        raise RuntimeError(f"Image gen failed: {err_text}")
    ...
    try:
        ...
        raise RuntimeError("Image gen result parse failed")

    files = result.get("files") or []
    if not files:
        raise RuntimeError("Image gen returned no file")
...
def get_agents():
    ...
    for a in agents:
        ...
        # 1) Auto-timeout not approved leave
        ...
        if auth_status == "approved" and last_push_at_str:
            try:
                ...
                if age > 300:  # 5mins no push auto-offline
...
def agent_approve():
    ...
    try:
        ...
        target["authExpiresAt"] = (datetime.now() + timedelta(hours=24)).isoformat()  # 24h
...
def join_agent():
    ...
    try:
        ...
        with join_lock:
            ...
            if key_expires_at_str:
                try:
                    ...
                    if datetime.now() > key_expires_at:
                        return jsonify({"ok": False, "msg": "This access key has expired，event has ended 🎉"}), 403
                except Exception:
                    pass
            ...
            #  key  3 
            # lastPushAt/updated_at  5 mins； offline
            now = datetime.now()
            ...
            if active_count >= max_concurrent:
                save_agents_state(agents)
                return jsonify({"ok": False, "msg": f"Access key concurrent limit reached（{max_concurrent}），please try later or use another key"}), 429
            ...
            if existing:
                ...
                existing["lastPushAt"] = datetime.now().isoformat()  # join /
        ...
        return jsonify({"ok": True, "agentId": agent_id, "authStatus": "approved", "nextStep": "Auto-approved，start pushing status immediately"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def leave_agent():
    ...
    try:
        ...
        if not target:
            return jsonify({"ok": False, "msg": "Not found to leave agent"}), 404
...
def agent_push():
    ...
    try:
        ...
        target = next((a for a in agents if a.get("agentId") == agent_id and not a.get("isMain")), None)
        if not target:
            return jsonify({"ok": False, "msg": "agent Not registered，please first join"}), 404
        ...
        if auth_status not in {"approved", "offline"}:
            return jsonify({"ok": False, "msg": "agent Not authorized，please wait for owner approval"}), 403
...
def assets_template_download():
    if not os.path.exists(ASSET_TEMPLATE_ZIP):
        return jsonify({"ok": False, "msg": "not foundplease generate first"}), 404
        ...
        with _bg_tasks_lock:
            _bg_tasks[task_id] = {
                ...
                "result": {
                    ...
                    "speed_mode": speed_mode,
                    "msg": "Generated and replaced RPG room background（auto-archived）",
                },
            }
    except Exception as e:
        msg = str(e)
...
def assets_generate_rpg_background():
    ...
    try:
        ...
        if not (os.path.exists(GEMINI_PYTHON) and os.path.exists(GEMINI_SCRIPT)):
            return jsonify({"ok": False, "msg": "Image gen script env missing：gemini-image-generate not installed"}), 500

        # Check if another generation is already running
        with _bg_tasks_lock:
            for tid, task in _bg_tasks.items():
                if task.get("status") == "pending":
                    return jsonify({"ok": True, "async": True, "task_id": tid, "msg": "Image gen task already in progress，please wait for completion"}), 200
        ...
        return jsonify({"ok": True, "async": True, "task_id": task_id, "msg": "Image gen task started，please poll via task_id for results"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def assets_generate_rpg_background_poll():
    ...
    if status == "pending":
        return jsonify({"ok": True, "status": "pending", "msg": "Image gen in progress..."})
    elif status == "done":
...
def assets_restore_reference_background():
    ...
    try:
        ...
        return jsonify({
            ...
            "path": "office_bg_small.webp",
            "size": st.st_size,
            "msg": "Restored original background",
        })
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def assets_restore_last_generated_background():
    ...
    try:
        ...
        if not os.path.isdir(BG_HISTORY_DIR):
            return jsonify({"ok": False, "msg": "No historical backgrounds"}), 404
        ...
        return jsonify({
            ...
            "msg": "Rolled back to last generated background",
        })
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def assets_home_favorites_save_current():
    ...
    try:
        ...
        return jsonify({"ok": True, "id": item_id, "path": os.path.relpath(dst, ROOT_DIR), "msg": "Current map favorited"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def assets_home_favorites_delete():
    ...
    try:
        ...
        _save_home_favorites_index(idx)
        return jsonify({"ok": True, "id": item_id, "msg": "Favorite deleted"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def assets_home_favorites_apply():
    ...
    try:
        ...
        st = target.stat()
        return jsonify({"ok": True, "path": "office_bg_small.webp", "size": st.st_size, "from": hit.get("path"), "msg": "ใช้แมปรายการโปรดแล้ว"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def assets_restore_prev():
    ...
    try:
        ...
        if not os.path.exists(bak):
            return jsonify({"ok": False, "msg": "Previous backup not found"}), 404

        shutil.copy2(str(target), bak + ".tmp") if os.path.exists(str(target)) else None
        shutil.copy2(bak, str(target))
        st = os.stat(str(target))
        return jsonify({"ok": True, "path": rel_path, "size": st.st_size, "msg": "Rolled back to previous version"})
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)}), 500
...
def assets_upload():
    ...
    try:
        ...
        if target.suffix.lower() not in ASSET_ALLOWED_EXTS:
            return jsonify({"ok": False, "msg": "Only image uploads allowed/art asset types"}), 400

        if not target.exists():
            return jsonify({"ok": False, "msg": "Target file not foundplease first /assets/list select path"}), 404
